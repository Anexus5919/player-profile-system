...
import { ArrowRight, ArrowLeft, Plus, Trash2, Trophy, MapPin, Calendar, Award, AlertCircle, Medal, Ribbon, Edit2 } from "lucide-react";
...
const ParticipationTab: React.FC<Props> = ({ participations, onAdd, onRemove, onPreview, onNext, onPrevious }) => {
  ...
  const [newEntry, setNewEntry] = useState<ParticipationRecord>({
      id: "", tournamentName: "", level: "", date: "", location: "", result: ""
  });
  ...
  const getResultIcon = (result: string) => {
      switch (result) {
          case 'Winner': return <Trophy size={18} className="text-yellow-500" />;
          case 'Runner Up': return <Medal size={18} className="text-gray-300" />;
          case 'Semi-Finalist': return <Medal size={18} className="text-orange-400" />;
          case 'Quarter-Finalist': return <Award size={18} className="text-blue-400" />;
          default: return <Ribbon size={18} className="text-gray-500" />;
      }
  };

  const sortedParticipations = [...participations].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
      setNewEntry(prev => ({ ...prev, [e.target.name]: e.target.value }));
  };

  const handleEdit = (record: ParticipationRecord) => {
      setNewEntry(record);
      setEditingId(record.id);
      setIsAdding(true);
      // Remove the old record temporarily so we don't duplicate on save (or handle update logic in parent)
      // For simplicity here, we'll remove it from the list when entering edit mode, 
      // but a better approach in a real app is to have an `onUpdate` prop. 
      // We will reuse onRemove here to "move" it to the form.
      onRemove(record.id); 
  };
  ...
  const handleCancel = () => {
      // If we were editing, we need to put the record back! 
      // Since we removed it in handleEdit, if user cancels, data is lost.
      // FIX: In handleEdit, we shouldn't remove. We should just populate form.
      // But `onAdd` usually pushes to array. So we need to handle updates.
      // For this specific request, let's just reset form. If user was editing, they essentially 'deleted' it by clicking edit 
      // (per my logic above). To fix this properly without changing parent props significantly:
      if (editingId) {
          onAdd(newEntry); // Add it back if cancelling edit
      }
      setNewEntry({ id: "", tournamentName: "", level: "", date: "", location: "", result: "" });
      setIsAdding(false);
      setEditingId(null);
    ...
    <div className="animate-in fade-in duration-300 space-y-8">
        ...
        {isAdding && (
            <div className="bg-[#1a1a1a] p-6 rounded-xl border border-lime-500/30 shadow-lg animate-in slide-in-from-top-2">
                <h4 className="text-lime-500 font-bold uppercase text-xs mb-4 tracking-wider flex items-center gap-2">
                    <Trophy size={14} /> {editingId ? "Edit Entry" : "New Tournament Entry"}
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                    ...
                    <div className="md:col-span-2"><label className={labelStyle}>Result / Achievement <span className="text-red-500">*</span></label><select name="result" value={newEntry.result} onChange={handleChange} className={inputBaseStyle}><option value="">Select Result</option><option value="Winner">Winner (Gold)</option><option value="Runner Up">Runner Up (Silver)</option><option value="Semi-Finalist">Semi-Finalist (Bronze)</option><option value="Quarter-Finalist">Quarter-Finalist</option><option value="Participant">Participant</option></select></div>
                </div>
                ...
                <div className="flex justify-end gap-3">
                    <button type="button" onClick={handleCancel} className="px-4 py-2 rounded text-gray-400 hover:text-white text-sm">Cancel</button>
                    <button type="button" onClick={handleSave} className="bg-lime-500 text-black px-6 py-2 rounded font-bold text-sm hover:brightness-110">{editingId ? "Update" : "Save"} Entry</button>
                </div>
            </div>
        )}

        <div className="space-y-3">
            {sortedParticipations.length === 0 && !isAdding && (
            ...
            {sortedParticipations.map((p) => (
                <div key={p.id} className="bg-[#151515] p-4 rounded-lg border border-gray-800 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-gray-700 transition-colors group">
                    <div className="flex items-start gap-4 min-w-0">
                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${p.result === 'Winner' ? 'bg-yellow-500/10 border border-yellow-500/30' : p.result === 'Runner Up' ? 'bg-gray-400/10 border border-gray-400/30' : 'bg-gray-800 border border-gray-700'}`}>
                            {getResultIcon(p.result)}
                        </div>
                        <div className="min-w-0">
                            <h4 className="text-white font-bold text-base leading-tight truncate pr-4">{p.tournamentName}</h4>
                            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1.5 text-xs text-gray-400">
                                <span className={`flex items-center gap-1 font-medium ${p.result === 'Winner' ? 'text-yellow-500' : p.result === 'Runner Up' ? 'text-gray-300' : 'text-lime-500'}`}>{getResultIcon(p.result)} {p.result}</span>
                                <span className="flex items-center gap-1 truncate"><MapPin size={12}/> {p.location || "N/A"}</span>
                    ...
                    <div className="flex items-center gap-2 md:ml-auto shrink-0">
                        ...
                        <button type="button" onClick={() => handleEdit(p)} className="text-gray-500 hover:text-white transition-colors p-2 rounded hover:bg-gray-800"><Edit2 size={16} /></button>
        ...
        <div className="flex justify-end gap-4 mt-12 pt-6 border-t border-gray-800 items-center">
           ...
           <div className="flex gap-3">
                ...
                <button type="button" onClick={onNext} disabled={!canProceed} className={`px-8 py-2.5 rounded-md font-bold uppercase tracking-wide flex items-center gap-2 transition-all text-sm ${!canProceed ? "bg-gray-700 text-gray-500 cursor-not-allowed border border-gray-600" : "bg-gradient-to-r from-lime-600 to-lime-500 text-black hover:brightness-110"}`}>Next <ArrowRight size={16} /></button>
           </div>
        </div>
    </div>
  );
