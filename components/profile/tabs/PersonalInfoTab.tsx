...
import { Image as ImageIcon, ChevronDown, FileText, Trash2, Eye, Info, AlertCircle, ArrowRight } from "lucide-react";
import { FormData, Units, IdentityFile } from "../CreateProfile";

// --- EXTENDED COUNTRY DATA WITH VALIDATION RULES ---
const COUNTRIES = [
...
const PersonalInfoTab: React.FC<Props> = ({ 
  formData, handleChange, handleUnitChange, handleNationalityChange, units, bmi, profilePic, identityFile, 
  ...
  useEffect(() => {
    ...
    if (!formData.sport) newErrors.push("Sport is required");
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} className={inputBaseStyle} placeholder="Enter your full name" />
            </div>
            ...
            {/* Contact No - DYNAMIC VALIDATION */}
            <div>
                <Label required>Contact No.</Label>
                <div className="flex">
                    <div className="bg-[#1e1e1e] border border-gray-700 border-r-0 rounded-l-md px-3 flex items-center text-gray-400 text-sm min-w-[80px] justify-center gap-2 select-none">
                    ...
                    <input 
                        ...
                        onChange={handleChange} 
                        className={`${inputBaseStyle} rounded-l-none`} 
                        placeholder={currentCountry.placeholder} // Dynamic Placeholder
                        maxLength={currentCountry.maxLen}       // Dynamic Constraint
                    />
                </div>
                {/* Helper text showing required length */}
                <p className="text-[10px] text-gray-600 mt-1 ml-1">
                    Required format: {currentCountry.minLen} digits for {currentCountry.name}
                </p>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <select name="gender" value={formData.gender} onChange={handleChange} className={`${inputBaseStyle} appearance-none`}>
            ...
            <div>
              ...
              <div className="relative">
                <select 
                    ...
                    onChange={(e) => {
                        const selected = COUNTRIES.find(c => c.nationality === e.target.value);
                        // Reset contact number when country changes to avoid validation conflicts
                        if(selected) {
                            handleNationalityChange(selected.nationality, selected.code);
                            // Optional: You might want to trigger a parent clear of contactNo here if needed
                        }
                    }} 
                ...
                >
                  ...
                  {COUNTRIES.map(c => (
                      <option key={c.name} value={c.nationality}>{c.nationality} ({c.name})</option>
                  ))}
                </select>
                <ChevronDown className="absolute right-3 top-3 text-gray-500 pointer-events-none" size={16} />
              </div>
            ...
            <div className="md:col-span-1">
              ...
              <textarea name="address" rows={1} value={formData.address} onChange={handleChange} className={`${inputBaseStyle} resize-none overflow-hidden h-[42px]`} />
            </div>

            {/* Sport Name */}
            <div>
              <Label required>Sport Name</Label>
              <div className="relative">
                <select name="sport" value={formData.sport} onChange={handleChange} className={`${inputBaseStyle} appearance-none`}>
                  <option value="">Select</option>
            ...
            <div className="md:col-span-2">
               ...
               <div className="flex gap-2">
                  <label className={`flex-1 cursor-pointer bg-[#2C2C2C] border rounded-md p-3 flex items-center gap-3 hover:border-lime-500/50 transition-colors ${!identityFile ? 'border-gray-700' : 'border-lime-500/50'}`}>
                     ...
                     <div className="flex flex-col"><span className="text-xs font-medium text-gray-300">{identityFile ? identityFile.name : "Browse"}</span><span className="text-[10px] text-gray-500">(pdf, jpg, png)</span></div>
                     <input type="file" className="hidden" accept=".pdf,image/*" onChange={onIdentityUpload} />
                  </label>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             ...
             <div>
                 ...
                 <div className="flex gap-2"><input type="number" name="height" value={formData.height} onChange={handleChange} className={inputBaseStyle} /><select value={units.height} onChange={(e) => handleUnitChange('height', e.target.value)} className="bg-[#2C2C2C] text-gray-400 text-xs rounded outline-none"><option value="cm">cm</option><option value="ft">ft</option></select></div>
             </div>
             
             {/* Weight */}
             <div>
                 <Label required>Weight</Label>
                 <div className="flex gap-2"><input type="number" name="weight" value={formData.weight} onChange={handleChange} className={inputBaseStyle} /><select value={units.weight} onChange={(e) => handleUnitChange('weight', e.target.value)} className="bg-[#2C2C2C] text-gray-400 text-xs rounded outline-none"><option value="kg">kg</option><option value="lbs">lbs</option></select></div>
             </div>
             
             {/* BMI */}
             <div>
                 <Label>BMI <InfoTooltip text="Calculated Body Mass Index" /></Label>
                 <div className="w-full bg-[#1e1e1e] rounded-md px-3 py-2.5 border border-gray-800 flex justify-between"><span className="text-gray-200 text-sm font-mono">{bmi.value || "--"}</span><span className={`text-xs font-bold uppercase ${bmi.color}`}>{bmi.status}</span></div>
             </div>
             
             {/* Dominant Hand */}
             <div>
                 <Label required>Dominant Hand <InfoTooltip text="Hand used for sports" /></Label>
                 <div className="relative"><select name="dominantHand" value={formData.dominantHand} onChange={handleChange} className={`${inputBaseStyle} appearance-none`}><option value="">Select</option><option value="Right">Right</option><option value="Left">Left</option></select><ChevronDown className="absolute right-3 top-3 text-gray-500 pointer-events-none" size={16} /></div>
             </div>
             
             {/* Disability */}
             <div>
                 <Label>Physical Disability (if any) <InfoTooltip text="Any permanent physical impairment" /></Label>
                 <div className="relative"><select name="disability" value={formData.disability} onChange={handleChange} className={`${inputBaseStyle} appearance-none`}><option value="No">No</option><option value="Yes">Yes</option></select><ChevronDown className="absolute right-3 top-3 text-gray-500 pointer-events-none" size={16} /></div>
             </div>
             
             {/* Description */}
             <div className="md:row-span-2">
                 <Label>Description (if yes)</Label>
                 <textarea name="disabilityDesc" value={formData.disabilityDesc} onChange={handleChange} disabled={formData.disability !== 'Yes'} className={`${inputBaseStyle} h-[116px] resize-none ${formData.disability !== 'Yes' ? 'opacity-50' : ''}`} />
             </div>
             
             {/* Optional Fields */}
             <div>
                 <Label>Wingspan (optional) <InfoTooltip text="Fingertip to fingertip length" /></Label>
                 <input type="text" name="wingspan" value={formData.wingspan} onChange={handleChange} className={inputBaseStyle} />
             </div>
             
             <div>
                 <Label>Agility Rating (optional) <InfoTooltip text="1-5 Scale of speed/coordination" /></Label>
                 <div className="relative"><select name="agilityRating" value={formData.agilityRating} onChange={handleChange} className={`${inputBaseStyle} appearance-none`}><option value="">Select</option>{[1,2,3,4,5].map(r => <option key={r} value={r}>{r}</option>)}</select><ChevronDown className="absolute right-3 top-3 text-gray-500 pointer-events-none" size={16} /></div>
             </div>
          </div>
        </div>
