...
const AchievementsTab: React.FC<Props> = ({ achievements, onAdd, onRemove, onPreview, onNext, onPrevious }) => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                const result = await upload(file, 'player-achievements');
                setNewEntry(prev => ({
                    ...prev,
    ...
    const canProceed = !isAdding && !uploading;
                ...
                <div className="bg-[#1a1a1a] p-6 rounded-xl border border-lime-500/30 shadow-lg animate-in slide-in-from-top-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        ...
                        <div className="md:col-span-2">
                            <label className={labelStyle}>Certificate (Optional)</label>
                            <div className="flex gap-2">
                                <label className={`flex-1 flex items-center justify-between bg-[#2C2C2C] rounded-md px-4 py-3 border border-dashed border-gray-600 hover:border-lime-500/50 cursor-pointer transition-colors group ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                                    <div className="flex items-center gap-3">
                                        <div className="bg-gray-800 p-2 rounded text-gray-400 group-hover:text-lime-500 transition-colors">
                                            <Upload size={16} />
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-sm text-gray-300 font-medium truncate max-w-[200px]">
                                                {uploading ? `Uploading... ${progress}%` : (newEntry.certificateName || "Upload Certificate")}
                                            </span>
                                            <span className="text-[10px] text-gray-500">Supported: PDF, JPG, PNG</span>
                                        </div>
                                    </div>
                                    <input type="file" className="hidden" accept=".pdf, .jpg, .jpeg, .png" onChange={handleFileChange} disabled={uploading} />
                                </label>
                                {newEntry.certificateUrl && !uploading && (
                                    <a href={newEntry.certificateUrl} target="_blank" rel="noreferrer" className="bg-[#2C2C2C] border border-gray-600 text-lime-500 w-12 rounded-md flex items-center justify-center hover:bg-gray-800 hover:border-lime-500 transition-all" title="Preview Certificate">
                                        <Eye size={20} />
                                    </a>
                                )}
                            </div>
