...
import { X, Activity, Mail, Phone, ImageIcon, Quote, ThumbsUp, ThumbsDown, Linkedin, Facebook, Twitter, Instagram, Trophy, Calendar, MapPin, Medal, Award, Ribbon } from "lucide-react";
...
const PieChart = ({ wins, loss, draws }: { wins: number, loss: number, draws: number }) => {
  ...
  const slices = [
    { label: "Wins", value: wins, color: "#84cc16" }, // Lime
    { label: "Loss", value: loss, color: "#ef4444" }, // Red
    { label: "Draws", value: draws, color: "#eab308" }, // Yellow
  ].filter(s => s.value > 0);
        ...
        {slices.map((slice, i) => {
          ...
          return (
            <path 
                ...
                onMouseEnter={() => {
                    setHoveredIndex(i);
                    setTooltipData({ label: slice.label, value: slice.value, percent: (slicePercent * 100).toFixed(1), color: slice.color });
                }}
                onMouseLeave={() => {
      ...
      {tooltipData && (
          <div className="absolute top-0 -right-16 z-20 bg-[#1a1a1a] p-3 rounded-lg border border-gray-700 shadow-2xl animate-in fade-in slide-in-from-left-2 duration-200 min-w-[100px]">
              <div className="flex items-center gap-2 mb-1">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: tooltipData.color }}></div>
                  <span className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">{tooltipData.label}</span>
              </div>
              <div className="flex items-baseline gap-1">
                  <span className="text-xl font-black text-white">{tooltipData.value}</span>
                  <span className="text-xs text-gray-500">matches</span>
              </div>
              <div className="mt-1 pt-1 border-t border-gray-800">
                  <span className="text-xs font-bold" style={{ color: tooltipData.color }}>{tooltipData.percent}%</span>
              </div>
          </div>
      )}
    </div>
      ...
      <div className="w-full max-w-3xl flex flex-col animate-in fade-in zoom-in duration-200">
        ...
        <div className="bg-[#121212] rounded-2xl overflow-hidden shadow-2xl border border-gray-800 relative group min-h-[450px] max-h-[85vh] overflow-y-auto custom-scrollbar">
           ...
           {activeTab === "BIO" ? (
                ...
                {data.sports.length > 0 ? (
                    <>
                        <div className="mb-8 w-full max-w-xs relative">
                            ...
                            <select value={selectedSport} onChange={(e) => setSelectedSport(e.target.value)} className="w-full bg-[#1a1a1a] text-white text-lg font-bold px-4 py-2 rounded-md border border-gray-700 focus:border-lime-500 outline-none appearance-none">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-16 w-full justify-center">
                            ...
                            <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                                ...
                                {extraStats.map(stat => (
                                    <div key={stat.label} className="bg-[#1a1a1a] p-4 rounded border border-gray-800 text-center"><span className="block text-xs text-gray-500 uppercase mb-1">{stat.label}</span><span className="text-xl font-bold text-white">{stat.value || "0"}</span></div>
                                ))}
                            </div>
                        </div>
                    </>
             ...
             /* --- PARTICIPATION PREVIEW (TIMELINE + ICONS) --- */
                             ...
                             <div key={i} className="relative pl-8">
                                 ...
                                 <div className="bg-[#1a1a1a] p-4 rounded-lg border border-gray-800 hover:border-gray-600 transition-colors">
                                     <div className="flex justify-between items-start mb-2">
                                         ...
                                         <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded border flex items-center gap-1 ${p.result === 'Winner' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' : p.result === 'Runner Up' ? 'bg-gray-400/10 text-gray-300 border-gray-400/20' : 'bg-gray-800 text-gray-500 border-gray-700'}`}>
                                             {getResultIcon(p.result)} {p.result}
                                         </span>
                                     </div>
                ...
                <div className="flex-1 w-full">
                   <div className="border-b border-gray-800 pb-4 mb-6">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-3">
                          ...
                          {data.sports.map(s => <span key={s} className="bg-gray-800 text-gray-300 px-2 py-1 rounded text-xs border border-gray-700 flex items-center gap-1">{s}</span>)}
                      </div>
                      <div className="mt-2 text-gray-500 text-xs uppercase tracking-wider font-medium">• {data.gender || "N/A"} • {calculateAge(data.dob)} Years Old</div>
                   </div>
